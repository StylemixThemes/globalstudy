...
if ( ! function_exists( 'globalstudy_sanitize_checkbox' ) ) {
	function globalstudy_sanitize_checkbox( $input ) {
 ...
 */
function globalstudy_menu_import() {
	...
	if ( ! empty( $menus ) ) {
		foreach ( $menus as $menu ) {
			$menu_names = array(
				'MS LMS globalstudy Theme Main Menu',
			);

			if ( is_object( $menu ) && in_array( $menu->name, $menu_names ) ) {
				$locations['ms-lms-globalstudy-theme-main-menu'] = $menu->term_id;
			}
		}
	}
...
add_action( 'globalstudy_merlin_after_all_import', 'globalstudy_menu_import' );

/**
 * Generating Pages for Theme Options
 */
function globalstudy_generate_lms_pages() {
...
if ( empty( get_option( 'stm_lms_settings' ) ) ) {
	add_action( 'globalstudy_merlin_after_all_import', 'globalstudy_generate_lms_pages' );
}

function elementor_set_default_settings_globalstudy() {
	//Elementor Settings
	...
	$from = trim( 'https://globalstudy.stylemixthemes.com/' );
	$to   = home_url();
...
add_action( 'globalstudy_merlin_after_all_import', 'elementor_set_default_settings_globalstudy' );

add_action( 'wp_enqueue_scripts', 'globalstudy_icons_script_styles' );
function globalstudy_icons_script_styles() {
...
add_action( 'admin_enqueue_scripts', 'globalstudy_admin_script_styles' );
function globalstudy_admin_script_styles() {
	wp_enqueue_style( 'admin_globalstudy_style', MS_LMS_GLOBALSTUDY_THEME_URI . '/assets/admin/admin_styles.css', '', MS_LMS_GLOBALSTUDY_THEME_VERSION );
}


add_action( 'admin_menu', 'globalstudy_show_nav_item' );
function globalstudy_show_nav_item() {

	add_menu_page(
		esc_html__( 'Welcome to Globalstudy Page', 'globalstudy' ),
		esc_html__( 'Globalstudy', 'globalstudy' ),
		'manage_options',
		'masterstudy-globalstudy-options',
		'globalstudy_admin_page_content',
		MS_LMS_GLOBALSTUDY_THEME_URI . '/assets/images/base/dash_global.svg',
		'2'
	);
}

function globalstudy_admin_page_content() {
	?>
	<div class="globalstudy-row">
		<div class="globalstudy-column has-content">
			<img src="<?php echo esc_url( MS_LMS_GLOBALSTUDY_THEME_URI . '/assets/images/base/globalstudy-left.png' ); ?>" alt="">
			<div class="content">
				<div class="logo">
					<img src="<?php echo esc_url( MS_LMS_GLOBALSTUDY_THEME_URI . '/assets/images/base/globalstudy_logo.svg' ); ?>" alt="">
				</div>
				<div class="text">
					Globalstudy is a starter theme from MasterStudy LMS plugin provides a complete and ready-made site template for eLearning and online education websites. The theme is fully responsive with a cutting-edge design that can be customized according to the user's requirements.
				</div>
				<div class="actions">
					<a href="https://docs.stylemixthemes.com/globalstudy-lms/" target="_blank" class="documentation globalstudy-wpcfto-documentation globalstudy-btn">
					...
					<a href="<?php echo admin_url() . esc_url( 'admin.php?page=globalstudy_lms_demo_installer' ); ?>" target="_blank" class="changelog globalstudy-transients globalstudy-btn">
						Demo Import
