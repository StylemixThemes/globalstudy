...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$this->base_path              = MS_LMS_GLOBALSTUDY_THEME_DIR;
...
//		wp_safe_redirect( admin_url( 'themes.php?page=merlin') );
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'globalstudy' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'globalstudy' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'globalstudy' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'globalstudy' ),
		...
		if ( $this->license_step_enabled ) {
			$this->steps['license'] = array(
				'name' => esc_html__( 'License', 'globalstudy' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'globalstudy' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'globalstudy' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['child'] = array(
			'name' => esc_html__( 'Child', 'globalstudy' ),
			'view' => array( $this, 'child' ),
		);

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'globalstudy' ),
	...
	protected function welcome() {
		...
		<div class="merlin__content--transition">
			...
			<?php echo '<img src="https://globalstudy.stylemixthemes.com/wp-content/uploads/2023/04/dark.svg" alt="Logo Globalstudy" class="globalstudy-lms-logo-merlin">' ?>
		...
		<footer class="merlin__content__footer">
			...
			?>
			<a href="<?php echo esc_url( $cancel_link ); ?>" class="globalstudy-lms-merlin merlin__button merlin__button--skip"><?php echo esc_html( $no ); ?></a>
			<a href="<?php echo esc_url( $this->step_next_link() ); ?>" class="globalstudy-lms-merlin merlin__button merlin__button--next merlin__button--proceed merlin__button--colorchange"><?php echo esc_html( $start ); ?></a>
			<?php wp_nonce_field( 'merlin' ); ?>
		</footer>

	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'globalstudy' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'globalstudy' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'globalstudy' ) );
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e( 'Required', 'globalstudy' ); ?>">
										<?php esc_html_e( 'Required *', 'globalstudy' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'globalstudy' ) );
	}
	...
	protected function content() {
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'globalstudy' ); ?>">
		...
		<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'globalstudy' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'globalstudy' ) );
	}
	/**
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'globalstudy' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'globalstudy' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'globalstudy' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'globalstudy' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'globalstudy' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'globalstudy' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'globalstudy' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'globalstudy' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'globalstudy' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'globalstudy' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'globalstudy' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'globalstudy' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'globalstudy' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'globalstudy' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'globalstudy' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'globalstudy' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'globalstudy' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'globalstudy' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'globalstudy' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'globalstudy' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'globalstudy' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'globalstudy' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'globalstudy' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'globalstudy' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'globalstudy' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'globalstudy' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'globalstudy' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'globalstudy' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'globalstudy' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'globalstudy' ),
				)
			);
		}
	...
	public function get_import_data_info( $selected_import_index = 0 ) {
		...
		if ( false !== has_action( 'globalstudy_merlin_after_all_import' ) ) {
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Demo Content', 'globalstudy' ),
				'description'      => esc_html__( 'Demo content data.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Elementor Widgets', 'globalstudy' ),
				'description'      => esc_html__( 'Sample widgets data.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'globalstudy' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Theme Options', 'globalstudy' ),
				'description'      => esc_html__( 'Sample theme options data.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'globalstudy' ),
				'description'      => esc_html__( 'Redux framework options.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
		...
		if ( false !== has_action( 'globalstudy_merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'globalstudy' ),
				'description'      => esc_html__( 'After import setup.', 'globalstudy' ),
				'pending'          => esc_html__( 'Pending', 'globalstudy' ),
				'installing'       => esc_html__( 'Installing', 'globalstudy' ),
				'success'          => esc_html__( 'Success', 'globalstudy' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'globalstudy' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'globalstudy' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( __( 'The home page was set', 'globalstudy' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'globalstudy' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'globalstudy' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'globalstudy' ), $import_file );
			}
		}
